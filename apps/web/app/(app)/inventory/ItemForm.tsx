...
export function ItemForm({ initialValues }: { initialValues?: Initial }) {
  ...
  // Live-preview the image URL as the owner types.
  const [imageUrl, setImageUrl] = useState(initialValues?.image_url ?? "");
  const previewUrl = /^https?:\/\//.test(imageUrl) ? imageUrl : null;

  return (
    <form action={action} className="grid md:grid-cols-[260px_1fr] gap-8">
      {isEdit && <input type="hidden" name="id" value={initialValues!.id} />}

      <div className="space-y-3">
        <ProductImage
          src={previewUrl}
          alt={initialValues?.name ?? "Product"}
        ...
        <label className="block space-y-1.5">
          ...
          <span className="text-[10px] text-[color:var(--color-muted)]">
            Setting this manually locks it against auto-enrichment.
          </span>
        </label>
      </div>
