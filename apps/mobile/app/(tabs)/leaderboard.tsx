...
export default function LeaderboardScreen() {
  ...
  const podiumOrder = top3.length >= 3
    ...
    : top3;
  const podiumHeights = [100, 140, 80]; // #2, #1, #3
      ...
      ListHeaderComponent={
        <>
          ...
          {top3.length >= 3 && (
            <View style={s.podium}>
              {podiumOrder.map((entry, i) => {
                ...
                const level = levelForStars(entry.total_stars);
                const crownSize = rank === 1 ? 28 : 20;
                const avatarSize = rank === 1 ? 64 : 52;
                const rankColors = { 1: "#C8984E", 2: "#9CA3AF", 3: "#B45309" };

                return (
                  <View key={entry.id} style={[s.podiumCol, { marginTop: rank === 1 ? 0 : 20 }]}>
                    <Text style={{ fontSize: crownSize, marginBottom: 4 }}>👑</Text>
                    <View style={[s.avatar, { width: avatarSize, height: avatarSize, borderColor: rankColors[rank as 1 | 2 | 3] }]}>
                      <Text style={[s.avatarText, { fontSize: avatarSize * 0.35 }]}>
                    ...
                    <View style={[s.podiumBar, { height, backgroundColor: rankColors[rank as 1 | 2 | 3] }]}>
                      <View style={[s.rankBadge, { backgroundColor: rankColors[rank as 1 | 2 | 3] }]}>
          ...
          {top3.length < 3 && top3.length > 0 && (
            <View style={s.earlyLeaderboard}>
              {top3.map((entry, i) => (
                <View key={entry.id} style={s.earlyRow}>
                  <Text style={s.earlyRank}>#{i + 1}</Text>
                  <Text style={s.earlyName}>{entry.users?.full_name ?? entry.users?.email ?? "—"}</Text>
                  <Text style={s.earlyStars}>⭐ {entry.total_stars}</Text>
                </View>
              ))}
            </View>
          )}
          ...
          {rest.length > 0 && (
            <View style={s.restHeader}>
              <Text style={s.restHeaderText}>Everyone else</Text>
            </View>
          )}
        </>
      }
        ...
        return (
          <View style={[s.row, isMe && s.rowMe]}>
            ...
            <View style={s.rowAvatar}>
              <Text style={s.rowAvatarText}>{name.slice(0, 2).toUpperCase()}</Text>
            </View>
            <View style={{ flex: 1 }}>
        ...
        top3.length === 0 ? (
          <View style={s.empty}>
            <Text style={s.emptyText}>
              No activity yet. Complete a quiz to appear on the leaderboard!
            </Text>
          </View>
        ) : null
      }
...
const s = StyleSheet.create({
  ...
  avatarText: { fontWeight: "700", color: colors.fg },
  ...
  earlyLeaderboard: { paddingHorizontal: 20, marginBottom: 24 },
  earlyRow: { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.border },
  earlyRank: { fontSize: 16, fontWeight: "700", width: 30 },
  earlyName: { flex: 1, fontSize: 15, fontWeight: "600" },
  earlyStars: { fontSize: 14, color: colors.gold, fontWeight: "600" },
